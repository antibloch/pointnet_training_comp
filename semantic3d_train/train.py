...
if not os.path.exists('reservoir'):
    ...
    # Modified to include only 2 classes: "background" and "man-made terrain"
    ...
    # Combine all labels except "man-made terrain" (label 1) into "background" (label 0)
    binary_labels = np.zeros_like(labels)
    binary_labels[labels == 1] = 1  # Only keep "man-made terrain" as 1, everything else as 0
    labels = binary_labels

    # After binary transformation, we only have 2 classes
    NUM_CLASSES = 2
    print("Binary classification: 0 = background, 1 = man-made terrain")
    ...
    # Save the binary class information
    old_labels = np.array([0, 1])
...
# Typically we might want to give more weight to the minority class
# Adjust weight based on class distribution
